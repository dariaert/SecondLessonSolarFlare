...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        output.setText(coreTime(delayMinute, volumeData, flash, coreFrequency) + " секунд");
    }
    ...
    private int volumeTime(int coreFrequency, int volumeData, int flash) {

        return ((volumeData * flash) / coreFrequency) / 60;
    ...
    private int coreTime(int[] delayMinute, int[] volumeData, int[] flash, int frequency) {
        ...
        for (int i = 0; i < volume.length; i++) { // инициализация данного массива
            volume[i] = volumeTime(frequency, volumeData[i], flash[i]); // определение времени обработки компьютером данных от одного спутника
        }
